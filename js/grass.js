//Rebuilt from Al-Ro's re-implementation of Eddie Lee's grass rendering
//2024 Jett Sjöberg - @JettIsOnTheNet
...
//Number of blades
var instances = mobile ? 50000 : 150000;
...
// Moon Texture
var moonTexture = loader.load("/assets/images/moon.webp");
...
//azimuth = (hour / 24) * Math.PI * 2;
azimuth = 0.1;

// Highest at noon (hour 12), lowest at midnight (hour 0 and 24)
//elevation = (Math.sin(((hour - 6) / 24) * Math.PI * 2) * Math.PI) / 2;
elevation = 0.1;

// Set the sun/moon texture
var texture = hour <= 6 || hour >= 20 ? moonTexture : null;

// Set the sun/moon position
var position = new THREE.Vector3(
  Math.sin(azimuth) * Math.sin(azimuth),
  Math.sin(elevation) * 1,
  -Math.cos(azimuth) * Math.cos(azimuth)
);

var position = new THREE.Vector3(210, 230, -2000);

// Sun or moon
var sunMoonMaterial = new THREE.MeshBasicMaterial({
  map: null, // Start with no texture. Default for sun
  side: THREE.DoubleSide,
});

// Update the sun/moon material
sunMoonMaterial.map = texture;
sunMoonMaterial.needsUpdate = true;

var fogFade = 0.005;

// Calculate brightness as a percentage of the day's progress
var ambientStrength = 0.3;
if (hour <= 6 || hour >= 20) {
  ambientStrength = 0;
} else if (hour <= 12) {
  // Increase from 7 to 12
  //ambientStrength = Math.max(0.3, (hour - 7) / (12 - 7));
  ambientStrength = Math.min(Math.max(0.1, (hour - 7) / (12 - 6)), 1);
} else {
  // Decrease from 12 to 19
  ambientStrength = Math.min(Math.max(0.1, 1 - (hour - 12) / (20 - 12)), 1);
}

//Lighting variables for grass
//var ambientStrength = 0.7;
var translucencyStrength = 0.8;
var specularStrength = 0.5;
var diffuseStrength = 0.5;
var shininess = 256;
var sunColour = new THREE.Vector3(
  ambientStrength,
  ambientStrength,
  ambientStrength
);
var specularColour = new THREE.Vector3(0.4, 0.4, 0.4);

// Colour for the ambient light
var skyColour = new THREE.Color(
  ambientStrength,
  ambientStrength,
  ambientStrength
);

// Colour brightness for time of day
skyColour.multiplyScalar(ambientStrength);

//Initialise three.js.
// There are two scenes which are drawn after one another
// with clear() called manually at the start of each frame
...
setCameraPosition(camera, 0, 7, 60);

scene.add(camera);
backgroundScene.add(camera);

//Light for ground plane
var ambientLight = new THREE.AmbientLight(skyColour, 0.5);
scene.add(ambientLight);

//Add the sun/moon
var sunMoonGeometry = new THREE.CircleGeometry(100, 32);
var sunMoon = new THREE.Mesh(sunMoonGeometry, sunMoonMaterial);
sunMoon.lookAt(camera.position);

// TODO: Set sun/moon position based on time of day
sunMoon.position.copy(position);
//sunMoon.position.set(position); //position);

hour <= 6 || hour >= 20 ? scene.add(sunMoon) : null;
...
var grassTexture = loader.load("/assets/images/blade_diffuse.webp");
var alphaMap = loader.load("/assets/images/blade_alpha.webp");
var noiseTexture = loader.load("/assets/images/perlin_noise.webp");
...
var groundMaterial = new THREE.MeshPhongMaterial({
  color: new THREE.Color("rgb(15%, 35%, 4%)"),
  emissive: new THREE.Color("rgb(2%, 6%, 1%)"),
...
// Helper function to set quaternion
function setQuaternion(quaternion, rotationAxis, angle) {
...
// Set quaternions
setQuaternion(quaternion0, new THREE.Vector3(0, 1, 0), 0.05);
setQuaternion(quaternion1, new THREE.Vector3(1, 0, 0), 0.3);
quaternion0.multiply(quaternion1);
setQuaternion(quaternion1, new THREE.Vector3(0, 0, 1), 0.1);
quaternion0.multiply(quaternion1);
...
let indices = Array.from({ length: instances }, (_, i) => i / instances);
let offsets = [];
let scales = [];
let halfRootAngles = [];
...
for (let i = 0; i < instances; i++) {
  ...
  scales.push(2.0 + Math.random() * (i % 3 != 0 ? 1.25 : 1));
}
